...
fpc_bep_result_t bep_enroll_finger(HCP_comm_t *chain)
{
  ...
  //  bmlite_on_start_enroll();
    ...
    LOG_DEBUG("bep_result%d",bep_result);

exit:
  //  bmlite_on_finish_enroll();
    LOG_DEBUG( "-> FPC - Enroll Finished.");
    LOG_DEBUG("Enroll samples");
    LOG_DEBUG("%d",((!enroll_done) ? FPC_BEP_RESULT_GENERAL_ERROR : bep_result));
